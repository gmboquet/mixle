...
- **Artifact under test:** `mixle-0.8.2-py3-none-any.whl` built with the pinned build closure from
  ...
  the commits that close the checklist after it change release records only, so the final candidate's
  wheel carries the same digest. Installed into a venv with the optional extras the corpus uses
