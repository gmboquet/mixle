...
A review against another commit or rebuilt bytes cannot authorize this candidate.
...
**Checks.**
- For each substantial example workflow (heterogeneous records, temporal/HMM, task
  cascade -- all synthetic-data since the 2026-08-04 dataset removal, D-0133), name the
  estimand: what quantity, over what population, under what sampling assumption?
...
*example workflow reports* state their estimand as precisely as the route layer does.

## Q2. Is validation leakage prevented?

**Checks.**
- Confirm the held-out / calibration / test separation is respected in the synthetic
  workflows under review, and that calibration touches only the calibration split.
  (Real-data workflows were removed from the repository 2026-08-04; review any
  notebook-based real-data claims where the notebooks live.)
...
- Confirm conformal calibration's exchangeability assumption is stated and not violated
  by the split (e.g. temporal order for the HMM workflows).

**Maintainer starting position.** The model-selection benchmark and the calibration
tests use held-out data; the reviewer should probe the *temporal* workflows hardest,
...
the same discipline in the example workflow reports.
...
## Q6. Do all stable distributions satisfy adverse mathematical contracts?

The reviewer must independently exercise every stable family in the maturity manifest:
Gaussian, Gamma, Exponential, LogGaussian, Poisson, Geometric, and Categorical. For each
applicable family, retain machine-readable pass/fail results for:

- normalization over the declared support and zero probability outside it;
- scalar, vectorized, encoded, and available engine score agreement;
- finite sampling, sampling/scoring support consistency, and deterministic seeded replay;
- estimator recovery from independently generated data;
- zero/fractional/effective weights and sufficient-statistic mass conservation;
- empty, degenerate, missing, malformed, negative-domain, and non-finite inputs;
- prior/posterior parameter validity where Bayesian routes exist;
- serialization score/sampling equivalence and mutation isolation.

Run the candidate's installed-wheel invariant catalog, but also retain at least one
reviewer-authored adverse probe per family so review is not merely replaying implementation
tests. The result artifact must name the full candidate source SHA, wheel digest, family,
contract, command, observed value/tolerance, and disposition. A skipped applicable contract
or an unexplained non-finite result is a release blocker.
...
Attach the per-family machine-readable invariant artifact and its SHA-256 here:

High-severity findings must be resolved or the claim reduced before RC1
(see the release-candidate-stages appendix of `0.8.0.md`, stage RC1 "freeze and independent review").
