...
|----|------|-------|---------|
| FC-1 | `forecast.py:140-165` → `price_forecast.paths` → `analysis/valuation.py` NPV | a | `paths` advertised as scenario draws for DCF have NO true cross-step dependence (per-step marginal multinomial) plus a spurious state-sorted ordering artifact: measured lag-1 regime corr 0.691 as returned, −0.025 after column shuffle, 0.960 true chain. Fix: forward-simulate genuine joint paths (per-step marginals are unchanged in distribution). |
