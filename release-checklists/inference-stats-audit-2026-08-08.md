...
| GATE-1 | `calibration_gate.py:124-134` | b,d | REPRODUCED IN-SESSION 2026-08-08 (k=200, bins=10): shipped threshold realizes level 0.0068 vs nominal 0.0100; across 60 alternative threshold seeds the realized level spans 0.0033–0.0181 — the level is an arbitrary constant set by seed 12345. Fix: the valid MC p-value (1+#{T_null ≥ T_obs})/(1+B), level-exact at any B; keep null_threshold for the score/power surfaces but never as the decision constant. Batch with GATE-4/GATE-7 (decision machinery) in one root-cause commit; GATE-2/GATE-6 are precondition docs, GATE-3 rides the CAL-1 finite-m fix. |
