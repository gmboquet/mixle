...
## Release-process items found while cutting 0.8.2

These are not findings against the library; they are defects in the release path that the 0.8.2 cut
measured and worked around, recorded so the next line does not rediscover them.

### P-01 · `full / py3.12 / shard 0` intermittently stalls at the tail of the shard on hosted runners
**Observed:** on the `tests.yml` dispatches for `909ec532` (run 35062092059) and `fca91130` (run 35071830883)
the shard printed its progress to `[ 99%]` in about 27 minutes, then nothing for 43 more, until the tier
harness killed it at its 4200-second budget (`full tier exceeded its 4200-second budget`, then the
`OSError: cannot send` teardown noise the budget kill always leaves). Each rerun passed. Green runs of the
same shard take 12-17 minutes (28 on `eb83ac59`), and the exact CI command -- `pytest -m full -n auto
--num-shards 4 --shard-id 0 --cov` -- passes locally in 17 minutes (3,580 passed, no stall). The tier receipt
records only duration, so which test was running when it stalled is not known.
**Cost:** each stall spends 70 minutes of a required lane before the rerun; at the final tag that is 70
minutes between a pushed tip and its dispatch completing.
**Next step:** make the tier harness dump the running tests (for example `faulthandler_timeout` well under
the budget, or per-worker "currently running" state) so the next stall names its test.

### P-02 · Dependabot's weekly run collides with the "No other open PR targets the release branch" gate
**Observed:** `.github/dependabot.yml` on `main` targets the active release line (#545, #546), so the weekly
run on 2026-09-16 opened six pull requests against `release/0.8.2` (#551-#556: a group of 11 minor/patch
bumps, `setuptools` and `readme-renderer` in the pinned build closure, and wider ceilings for `tbb`,
`sentence-transformers` and `sphinx-polyversion`). The release owner closed them as deferred past 0.8.2 so the
gate could be measured passing.
**Next step:** point `target-branch` at `main` once 0.8.2 ships, so dependency updates land on the
development line and a release branch only takes what is deliberately backported.
