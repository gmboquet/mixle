...
* `python -m mixle.ppl.benchmark_vs` — **head-to-head against the actual competing PPLs**
...
mixture-of-conjugate models, mixle.ppl isn't "competitive with" sampling PPLs — it sidesteps
...
* **Mixing efficiency (ESS/sec)**: mixle's affine-invariant **ensemble** sampler is the leader —
...
* **Single-fit wall-clock**: mixle RW-Metropolis is fastest to *an* answer (639 ms) — no compile
...
* **General non-conjugate posteriors**: mixle's `ensemble` sampler has the **highest ESS/sec** of
