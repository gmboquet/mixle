...
The single subsurface-posterior type both a core `mixle` distribution and a `mixle_pde` field posterior
satisfy, so the spine (E1-E11), the stochastic optimizer (H4), and the valuation (J2) all consume one
structural type. `samples` is a METHOD returning an ``(n, d)`` draw matrix (NOTE the plural --
`mixle_pde.latent.PosteriorField3D.sample` is singular today; E1 adds the plural alias). `cov` returns a
dense array for small problems or a `LinearOperator` for survey-scale ones (never materialised -- work-plan
C2). `derived_quantity` maps a pushforward ``fn`` over posterior draws into a `DerivedQuantity` that carries
the samples, a credible interval, AND the honesty flag `prior_dominated` (work-plan A2): a driller-facing
number is never emitted without it.

This module was scoped for Wave 0 (a frozen, build-against-contract stub other tasks import) but had not
actually landed on ``release/0.8.0`` when E1 started; E1 lands it here verbatim from the frozen contract
text (``notes/exec/contracts.md`` IC-1) as a prerequisite so its own conformance work has a protocol to
satisfy.
"""
