...
The native :mod:`mixle.engines.symbolic_engine` produces a dependency-free
...
Both backends are imported lazily so importing :mod:`mixle.engines` never
...
from mixle.engines.symbolic_engine import SymbolicExpression
...
def _import_sympy():
    try:
        ...
        raise ImportError(
            ...
            "`pip install sympy` (or `pip install mixle[sympy]`)."
        ) from exc
    return sympy
