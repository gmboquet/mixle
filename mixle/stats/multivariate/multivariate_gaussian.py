...
class MultivariateGaussianAccumulator(SequenceEncodableStatisticAccumulator):
    ...
    def update(self, x: np.ndarray, weight: float, estimate: MultivariateGaussianDistribution | None) -> None:
        ...
        # first observation THAT CARRIES POSITIVE WEIGHT (an O(d^2) bookkeeping track on a path that
        # is already O(d^2)). A weight of exactly 0.0 -- an EM component's responsibility for a point
        # it does not own, ordinary usage of this calling convention -- contributes nothing to either
