...
def marginalize_estimator_leaves(estimator: Any, missing_value: Any = MISSING) -> Any:
    ...
    value is integrated out). Used by ``mixle.ppl`` to fit a model from data with missing entries without
    imputing them."""
    ...
    if isinstance(estimator, CompositeEstimator):
        ...
        children = [marginalize_estimator_leaves(child, missing_value) for child in estimator.estimators]
...
def unwrap_marginalized(dist: Any) -> Any:
    ...
    if isinstance(dist, CompositeDistribution):
        ...
        return unwrapped
    if isinstance(dist, OptionalDistribution) and getattr(dist, "_marginalized_by_helper", False):
