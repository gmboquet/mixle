...
class UniformDistribution(SequenceEncodableProbabilityDistribution):
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        """Keep the cached ``log_density_value`` tied to the parameters ``low`` and ``high`` they derive from.
        ...
        if name not in ("low", "high"):
            return
        try:
            object.__setattr__(self, "log_density_value", -math.log(self.high - self.low))
            ...
            object.__setattr__(self, "log_density_value", float("nan"))
