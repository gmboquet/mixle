...
class InverseGammaDistribution(SequenceEncodableProbabilityDistribution):
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        """Keep ``log_const`` tied to the parameter(s) ``alpha``/``beta`` they derive from.
        ...
        if name not in ("alpha", "beta"):
            return
        try:
            object.__setattr__(self, "log_const", float(self.alpha * math.log(self.beta) - gammaln(self.alpha)))
