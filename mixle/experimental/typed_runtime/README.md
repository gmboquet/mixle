...
Optimization evidence is generated by `python benchmarks/typed_optimization.py --reps 5`. The benchmark keeps
fixed-iteration parity, target-matched time, removed work, batch semantics, and target failures separate; current
results show a real E-step-reuse win, but no wall-time win yet for selective block scheduling or geometry routing.
