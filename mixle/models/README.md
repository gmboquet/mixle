...
| --- | --- | --- |
| **Neural & deep** | `neural`, `neural_leaf`, `neural_density`, `softmax_leaf`, `transformer`, `language_model`, `streaming_transformer_leaf`, `embedding`, `dpo_leaf`, `continual`, `train_search` | Neural networks as mixle leaves — **adapters** that make a torch model a composable `Distribution`: `NeuralLeaf` wraps a conditional net (`p(y\|x)`), `NeuralDensityLeaf` wraps *any* unconditional density module (e.g. a normalizing flow via `build_coupling_flow`) so it drops into a mixture/HMM/composite and fits jointly with classical families by EM. Plus a causal-Transformer LM, a shared `CategoricalEmbedding`, and training utilities — DPO, continual-learning (EWC), and multi-fidelity DoE over the training recipe (`tune_training`). The rule: add neural models when they're new *distributions*, not new architectures. |
