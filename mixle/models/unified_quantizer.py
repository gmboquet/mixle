...
**Vendoring note (F11, version skew against this worktree's ``mixle/task/quantize.py``).** This
module was pulled across from ``origin/unified-quantizer`` (I1, byte-for-byte per the same
``git show <branch>:<path>`` vendoring precedent :mod:`mixle.task.checkpoint_family_ladder`'s own
docstring documents) for F11 (deployment of the family), which needs I1's per-tensor auto-pick
quantizer to build the "I-quantized artifacts" it produces. ``mixle/task/quantize.py`` on this
worktree's base (``origin/checkpoint-family-ladder``) has ITSELF evolved independently of
``origin/unified-quantizer``'s copy: the function this module originally imported as two names,
``symmetric_quantize`` (quantize) and ``dequantize_symmetric`` (dequantize), was refactored upstream
on this branch into ONE function, ``quantize_dequantize_array(w, *, bits=8, clip_percentile=None) ->
(wq, scale)`` -- same arithmetic, same return shape, ``bits`` now keyword-only, no separate dequantize
helper (callers multiply ``wq.astype(float64) * scale`` themselves, exactly what this module's own
``SymmetricQuantPayload.reconstruct`` already did via the old helper). Diffed side-by-side: the
quantize half is byte-identical math, just renamed/reshaped; the dequantize half is the one-line
multiply this module already performed. So rather than reintroducing the two old names (which would
silently diverge from the one real implementation this worktree's ``quantize.py`` now has), the call
site below uses ``quantize_dequantize_array`` directly and ``SymmetricQuantPayload.reconstruct`` does
the multiply inline -- no arithmetic was reimplemented, only the import/call site was adapted to the
current function name/signature.
"""
...
from mixle.task.quantize import quantize_dequantize_array as _quantize_dequantize_array
...
class SymmetricQuantPayload:
    ...
    def reconstruct(self) -> np.ndarray:
        # mixle.task.quantize no longer exposes a standalone dequantize_symmetric on this branch --
        # see the module docstring's vendoring note. The multiply is exactly what that helper did.
        return np.asarray(self.wq, dtype=np.float64) * float(self.scale)
...
def _run_int(flat: np.ndarray, bits: int, clip_percentile: float | None) -> tuple[SymmetricQuantPayload, np.ndarray]:
    wq, scale = _quantize_dequantize_array(flat, bits=bits, clip_percentile=clip_percentile)
