...
def _bytecode_matches_source(pyc: Path, source: Path) -> bool | None:
    ...
    the filename and optimization level the ``.pyc`` itself records. Measured against real
    pip-generated bytecode this reproduces byte-for-byte; ``None`` is returned when the comparison
    cannot be performed at all (unreadable, truncated, or a marshal format this interpreter cannot
    load), so the caller can treat "undecidable" differently from "wrong".
    """
    import marshal
