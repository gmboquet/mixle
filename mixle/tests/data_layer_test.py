...
class StructureCapabilityTest(unittest.TestCase):
    def test_supported_structures_inference(self):
        from mixle.data.structure import supported_structures
...
class ConnectorTest(unittest.TestCase):
    ...
    def test_jsonl_records(self):
        ...
        from mixle.data import open_source
...
class ColdImportTest(unittest.TestCase):
    """``import mixle.data`` must work standalone. The graph adapter (GraphDataEncoder) subclasses a
    contract under mixle.stats whose graph distributions import the adapter back, so eager re-export
    used to deadlock when mixle.data was imported before mixle.stats. Each case runs in a fresh
    ...
    def test_import_pysp_data_cold(self):
        self._run("import mixle.data")

    def test_cold_graph_adapter_access_without_stats_first(self):
        self._run("from mixle.data import GraphDataEncoder; assert GraphDataEncoder().directed is False")

    def test_cold_star_import(self):
        self._run("from mixle.data import *; assert GraphObservation is not None and Schema is not None")
