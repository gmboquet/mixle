"""Tests for mixle.capability — feature detection by what an object supports, not its class."""
...
def test_distribution_capabilities_method():
    # dist.capabilities() is the public UX surface; equivalent to mixle.capabilities(dist).
    d = _cat()
    assert d.capabilities() == cap.capabilities(d)
    import mixle

    assert mixle.supports(d, cap.Enumerable) and mixle.capabilities(d) == d.capabilities()
...
def test_describe_is_plain_english_and_capability_accurate():
    ...
    mix_text = mixle.describe(MixtureDistribution([GaussianDistribution(0, 1), GaussianDistribution(3, 1)], [0.5, 0.5]))
...
def test_describe_is_robust_on_classes_and_non_distributions():
    ...
    assert isinstance(mixle.describe(GaussianDistribution), str)  # the class
    assert "RandomForestEstimator" in mixle.describe(mixle.models.RandomForestEstimator)
    assert "can:" in mixle.describe(GaussianDistribution(0, 1))  # the instance is still rich
...
def test_ws4_distribution_capabilities():
    ...
    import mixle
    from mixle.capability import Continuous, Discrete, Fittable, HasCDF, Optimizable
    from mixle.relations import Assignment
    from mixle.stats.univariate.discrete.categorical import CategoricalDistribution

    g = GaussianDistribution(0.0, 1.0)
    c = CategoricalDistribution({"a": 0.5, "b": 0.5})
    assert mixle.supports(g, HasCDF) and mixle.supports(g, Continuous) and not mixle.supports(g, Discrete)
    assert mixle.supports(c, Discrete) and not mixle.supports(c, Continuous)
    assert mixle.supports(g, Fittable) and mixle.supports(c, Fittable)
    assert mixle.supports(Assignment(np.array([[1.0, 2.0], [3.0, 4.0]])), Optimizable)
    for d in (g, c):  # Discrete and Continuous are mutually exclusive
        assert not (mixle.supports(d, Discrete) and mixle.supports(d, Continuous))


def test_catalog_is_the_single_vocabulary():
    import mixle

    names = {s.name for s in mixle.catalog()}
...
def test_what_supports_filters_by_capability():
    ...
    assert mixle.what_supports(cap.Conditionable, pool) == ["MultivariateGaussianDistribution"]
