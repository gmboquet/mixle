...
def _make_candidate(root: Path) -> tuple[str, str, str, str]:
    ...
    brief = (
        "# mixle candidate\n\n"
        f"- Source commit: `{commit}` (branch release/0.8.2, exact tip)\n"
        f"- Source tree: `{tree}`\n"
    ...
    (root / "release-candidate-attestation.json").write_text(
        ...
        '"candidate_tree":"%s","version":"0.8.2","wheel_sha256":"%s","sdist_sha256":"%s",'
