...
Deploying And Operating Mixle Core
----------------------------------
...
Record partial distributed state, make retries idempotent, and verify
artifact identity before resume. Roll back by restoring the prior package and
compatible artifact/schema set; do not rewrite shared history.

Support
~~~~~~~

Supported versions and reporting channels are in ``../SECURITY.md``,
``../README.md``, and the release notes. Optional backend support is bounded
by the declared matrix and evidence, not by the presence of an import adapter.
