...
class POMDPModel(object):
    ...
    def __init__(self, transition: Any, observation: Any,
                 initial_belief: Optional[Any] = None,
    ...
    def __str__(self) -> str:
        return 'POMDPModel(num_states=%d, num_actions=%d, num_observations=%d, name=%r)' % (
            self.num_states, self.num_actions, self.num_observations, self.name)
    ...
    def filter(self, actions: Sequence[int], observations: Sequence[int],
               initial_belief: Optional[Any] = None) -> POMDPFilterResult:
        ...
        belief = self.initial_belief if initial_belief is None else _as_simplex(
            initial_belief, self.num_states, 'initial_belief')
    ...
    def sequence_log_likelihood(self, actions: Sequence[int], observations: Sequence[int],
                                initial_belief: Optional[Any] = None) -> float:
        """Return log P(observations | actions, model)."""
        return self.filter(actions, observations, initial_belief).log_likelihood

    def forward_backward(self, actions: Sequence[int], observations: Sequence[int],
                         initial_belief: Optional[Any] = None) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        init = self.initial_belief if initial_belief is None else _as_simplex(
            initial_belief, self.num_states, 'initial_belief')
    ...
    def sample(self, actions: Sequence[int], seed: Optional[int] = None,
               initial_belief: Optional[Any] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _forward_scaled(self, actions: np.ndarray, observations: np.ndarray,
                        initial_belief: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _backward_scaled(self, actions: np.ndarray, observations: np.ndarray,
    ...
    def _transition_marginals(self, actions: np.ndarray, observations: np.ndarray,
                              initial_belief: np.ndarray, alpha: np.ndarray,
                              beta: np.ndarray) -> np.ndarray:
        xi = np.empty((len(actions), self.num_states, self.num_states), dtype=np.float64)
        for t, (a, o) in enumerate(zip(actions, observations)):
            prev = initial_belief if t == 0 else alpha[t - 1]
            mat = (prev[:, None] * self.transition[int(a)]
                   * (self.observation[int(a), :, int(o)] * beta[t])[None, :])
            total = mat.sum()
...
def baum_welch_pomdp(sequences: Sequence[Tuple[Sequence[int], Sequence[int]]],
                     num_states: int,
                     num_actions: int,
                     num_observations: int,
                     initial_model: Optional[POMDPModel] = None,
                     max_its: int = 50,
                     tol: Optional[float] = 1.0e-8,
                     pseudo_count: float = 1.0e-3,
                     seed: Optional[int] = None) -> POMDPFitResult:
...
def _random_pomdp(num_states: int, num_actions: int, num_observations: int,
                  rng: np.random.RandomState) -> POMDPModel:
