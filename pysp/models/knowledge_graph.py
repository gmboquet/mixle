...
class TransEKnowledgeGraphModel(object):
    """Dependency-free TransE model with a NumPy margin objective."""

    def __init__(self, entity_embeddings: Any, relation_embeddings: Any,
                 entity_names: Optional[Sequence[Any]] = None,
                 relation_names: Optional[Sequence[Any]] = None,
    ...
    @classmethod
    def random(cls, num_entities: int, num_relations: int, embedding_dim: int = 16,
               seed: Optional[int] = None, scale: float = 0.01,
               entity_names: Optional[Sequence[Any]] = None,
               relation_names: Optional[Sequence[Any]] = None,
               name: Optional[str] = None) -> 'TransEKnowledgeGraphModel':
    ...
    def __str__(self) -> str:
        return 'TransEKnowledgeGraphModel(num_entities=%d, num_relations=%d, dim=%d, name=%r)' % (
            self.num_entities, self.num_relations, self.embedding_dim, self.name)
    ...
    def margin_loss(self, positive_triples: Sequence[Triple], negative_triples: Sequence[Triple],
    ...
    def negative_sample(self, triples: Sequence[Triple], seed: Optional[int] = None,
                        corrupt: str = 'tail') -> List[Triple]:
    ...
    def fit_margin(self, positive_triples: Sequence[Triple],
                   negative_triples: Optional[Sequence[Triple]] = None,
                   margin: float = 1.0,
                   lr: float = 0.01,
                   max_its: int = 100,
                   seed: Optional[int] = None,
                   normalize_entities: bool = True) -> KnowledgeGraphFitResult:
        ...
        for _ in range(max(1, int(max_its))):
            negatives = list(negative_triples) if negative_triples is not None else self.negative_sample(
                positives, seed=int(rng.randint(0, 2 ** 31 - 1)), corrupt='both')
