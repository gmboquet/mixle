...
class BesselTestCase(unittest.TestCase):
    ...
    def test_lniv_matches_mpmath(self):
        ...
        cases = [(0.5, 1.0), (0.5, 50.0), (1.0, 1e-6), (4.5, 2.0), (4.5, 800.0),
                 (49.0, 1.0), (49.0, 3000.0), (200.0, 10.0),
                 (500.0, 1.0), (1000.0, 10.0), (300.0, 0.01)]
        for v, z in cases:
...
class NormalizationTestCase(unittest.TestCase):
    ...
    def test_general_dim_against_mpmath(self):
        for dim, k in [(2, 3.0), (5, 0.7), (10, 25.0), (50, 4.0)]:
            mu = np.zeros(dim); mu[0] = 1.0
            d = VonMisesFisherDistribution(list(mu), k)
            v = dim / 2.0 - 1.0
            exact = float(v * mpmath.log(k) - (dim / 2.0) * mpmath.log(2 * mpmath.pi)
                          - mpmath.log(mpmath.besseli(v, k)))
...
class SamplerTestCase(unittest.TestCase):
    ...
    def test_unit_norm_and_moments(self):
        for dim, k in [(3, 5.0), (8, 10.0), (3, 0.5)]:
            ...
            self.assertLess(abs((x @ mu).mean() - a_exact), 0.02,
                            'dim=%d kappa=%g' % (dim, k))
