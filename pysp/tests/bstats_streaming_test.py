...
def _posterior_stream_cases():
    def poisson_case():
        ...
        return (PoissonDistribution(2.0, prior=prior),
                PoissonEstimator(prior=prior),
                [0, 1, 3, 2],
                [4, 1, 0])

    def exponential_case():
        prior = GammaDistribution(2.0, 0.25)
        return (ExponentialDistribution(1.2, prior=prior),
    ...
    def binomial_case():
        ...
        return (BinomialDistribution(10, 0.4, prior=prior),
                BinomialEstimator(10, prior=prior),
                [3, 7, 5, 2],
                [8, 4, 1])

    def geometric_case():
        prior = BetaDistribution(2.0, 3.0)
        return (GeometricDistribution(0.4, prior=prior),
                GeometricEstimator(prior=prior),
                [1, 2, 3, 1],
                [4, 2])

    def categorical_case():
        prior = DictDirichletDistribution({'a': 2.0, 'b': 1.5, 'c': 1.0})
        return (CategoricalDistribution({'a': 0.5, 'b': 0.3, 'c': 0.2}, prior=prior),
    ...
    def integer_categorical_case():
        ...
        return (IntegerCategoricalDistribution([0.2, 0.5, 0.3], min_index=2, prior=prior),
    ...
    def optional_case():
        ...
        return (OptionalDistribution(GaussianDistribution(0.0, 1.0, prior=child_prior),
                                     p=0.25, prior=prior),
...
class BayesianStreamingEstimatorTestCase(unittest.TestCase):
    ...
    def assertPriorClose(self, actual, expected):
        ...
        if isinstance(actual_params, dict):
            ...
            for key in actual_params:
                np.testing.assert_allclose(np.asarray(actual_params[key], dtype=float),
                                           np.asarray(expected_params[key], dtype=float),
                                           rtol=1.0e-12, atol=1.0e-12)
            return

        np.testing.assert_allclose(np.asarray(actual_params, dtype=float),
                                   np.asarray(expected_params, dtype=float),
                                   rtol=1.0e-12, atol=1.0e-12)
    ...
    def test_mixture_posterior_carry_updates_weight_and_component_priors(self):
        ...
        np.testing.assert_allclose(model.get_prior().dists[0].get_parameters(),
                                   expected.get_prior().dists[0].get_parameters(),
                                   rtol=1.0e-12, atol=1.0e-12)
        for actual_prior, expected_prior in zip(model.get_prior().dists[1].dists,
                                                expected.get_prior().dists[1].dists):
    ...
    def test_nested_estimator_scalers_preserve_child_support_metadata(self):
        ...
        hmm_est = HiddenMarkovModelEstimator(
            ...
            prior=(DirichletDistribution(np.ones(2)),
    ...
    def test_posterior_carry_across_conjugate_families(self):
        for name, factory in _posterior_stream_cases():
            with self.subTest(family=name):
                ...
                expected1 = expected_estimator.estimate(
                    _accumulate(expected_estimator, expected_start, batch1))
                self.assertPriorClose(model1.get_prior(), expected1.get_prior())

                model2 = stream.update(batch2)
                expected2_estimator = expected1.estimator()
                expected2 = expected2_estimator.estimate(
                    _accumulate(expected2_estimator, expected1, batch2))
                self.assertPriorClose(model2.get_prior(), expected2.get_prior())
                self.assertPriorClose(stream.estimator.get_prior(), model2.get_prior())
    ...
    def test_forgetting_across_conjugate_families(self):
        ...
        for name, factory in _posterior_stream_cases():
            with self.subTest(family=name):
                ...
                expected = expected_estimator.estimate(
                    _scale_for_estimator(expected_estimator, raw_stats, rho))
