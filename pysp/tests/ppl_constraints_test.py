...
class ConstrainedInferenceTestCase(unittest.TestCase):
    ...
    def test_mcmc_respects_ordering(self):
        m, a, b = self._model()
        fit = m.fit(self.data, how="mcmc", constraints=a < b,
                    draws=1200, burn=400, rng=np.random.RandomState(1))
    ...
    def test_ensemble_respects_ordering(self):
        m, a, b = self._model()
        fit = m.fit(self.data, how="ensemble", constraints=a < b,
                    draws=800, burn=300, rng=np.random.RandomState(2))
        self.assertLess(fit.params["a"], fit.params["b"])
    ...
    def test_reversed_constraint_forces_boundary(self):
        ...
        m, a, b = self._model()
        fit = m.fit(self.data, how="ensemble", constraints=a > b,
                    draws=800, burn=300, rng=np.random.RandomState(3))
