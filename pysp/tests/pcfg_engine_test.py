...
from pysp.stats import (HeterogeneousPCFGDistribution, GaussianDistribution, StudentTDistribution,
        ...
        for name, engine in self.engines:
            with self.subTest(engine=name):
                ...
                self.assertTrue(np.allclose(got, ref, atol=1.0e-9),
                                '%s inside score disagrees with seq_log_density' % name)
