...
from pysp.utils.enumeration import freeze, rerank_by_density, sound_top_k, stable_top_k, supports_enumeration
...
class BruteForceCrossCheckTestCase(unittest.TestCase):
    ...
    def test_sound_top_k_correct_for_nested_mixture(self):
        # The case where stable_top_k is UNSOUND (mixture of nested composite/sequence models):
        # sound_top_k must return the exact true-descending top-k AND an arbitrary-index slice,
        # via its probability-mass certificate (independent of the tropical stream ordering).
