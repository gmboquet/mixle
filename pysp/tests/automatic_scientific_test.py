...
from pysp.stats import (
    BernoulliSetEstimator, CompositeEstimator, estimate, GaussianEstimator, IgnoredEstimator,
    initialize, IntegerCategoricalEstimator, PoissonEstimator, seq_encode, seq_estimate,
    seq_log_density_sum, SequenceEstimator,
)
...
class AutomaticScientificProfilingTestCase(unittest.TestCase):
    ...
    def test_marginal_model_selection_synthetic_grid(self):
        ...
        cases = [
            ('dense_integer', [0, 1, 2, 1] * 100,
             IntegerCategoricalEstimator, 'integer_categorical'),
            ('poisson_count', list(rng.poisson(30, 500)),
             PoissonEstimator, 'poisson'),
            ('wide_signed_integer', list(rng.randint(-1000, 1000, 500)),
             GaussianEstimator, 'gaussian'),
            ('sparse_integer_identifier', [10_000_000 + 1000 * i for i in range(200)],
             IgnoredEstimator, 'ignored'),
        ]
    ...
    def test_auto_estimators_round_trip_through_vectorized_estimation_matrix(self):
        ...
        cases = [
            ...
            ('nested_dict', [{'meta': {'kind': 'a'}, 'value': 0},
                             {'meta': {'kind': 'b'}, 'value': 1},
                             {'value': 2}] * 40),
    ...
    def test_marginal_validation_is_bounded_and_optional(self):
        ...
        profile = analyze_structure(data, pairwise=False, validation_fraction=0.50,
    ...
    def test_integer_categorical_validation_handles_wide_dense_support(self):
        ...
        profile = analyze_structure(data, pairwise=False, validation_fraction=0.5,
                                    max_validation_rows=25)
        field = profile.fields[0]
    ...
    def test_marginal_validation_is_deterministic_and_summary_serializable(self):
        ...
        p1 = analyze_structure(data, pairwise=True, max_pairwise_pairs=4,
                               validation_seed=123, rng=np.random.RandomState(5))
        p2 = analyze_structure(data, pairwise=True, max_pairwise_pairs=4,
                               validation_seed=123, rng=np.random.RandomState(5))
    ...
    def test_marginal_validation_disagreement_is_explained(self):
        data = [0] * 30 + [1] * 10 + [10] * 2
        profile = analyze_structure(data, pairwise=False, validation_seed=17,
                                    validation_fraction=0.25, validation_min_count=10)
        field = profile.fields[0]

        self.assertEqual(field.recommendation, 'integer_categorical')
        self.assertEqual(field.validation_recommendation, 'poisson')
        self.assertIn('validation prefers poisson over marginal recommendation integer_categorical',
    ...
    def test_independent_product_has_no_dependency_tree(self):
        ...
        data = [
            (int(rng.randint(0, 2)), int(rng.randint(0, 3)), int(rng.randint(0, 4)))
            for _ in range(1000)
        ]
    ...
    def test_stratified_pair_budget_can_find_late_field_dependency(self):
        ...
        profile = analyze_structure(data, pairwise=True, max_pairwise_fields=20,
                                    max_pairwise_pairs=10, mi_threshold_bits=0.1)
        hint_pairs = {frozenset((hint.left, hint.right)) for hint in profile.pairwise_hints}
    ...
    def test_sampling_and_pairwise_budget_are_reported(self):
        ...
        profile = analyze_structure(data, pairwise=True, sample_size=200,
                                    max_pairwise_fields=4, max_pairwise_pairs=3,
                                    mi_threshold_bits=0.0, rng=np.random.RandomState(14))
