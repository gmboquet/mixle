...
from pysp.stats import (
    ...
    ExponentialDistribution, ExponentialEstimator, GaussianDistribution, GaussianEstimator,
    GeometricDistribution, GeometricEstimator, IntegerCategoricalDistribution,
    IntegerCategoricalEstimator, MixtureDistribution, MixtureEstimator, PoissonDistribution,
    PoissonEstimator, SequenceDistribution, SequenceEstimator,
...
def make_mixture(K=3):
    ...
    for k in range(K):
        ...
        comps.append(CompositeDistribution((
            ...
            SequenceDistribution(IntegerCategoricalDistribution(0, list(np.roll([0.5, 0.3, 0.1, 0.1], k))),
...
def make_estimator(K=3):
    comp = CompositeEstimator((
        ...
        SequenceEstimator(IntegerCategoricalEstimator(min_val=0, max_val=3),
...
class KernelsTestCase(unittest.TestCase):
    ...
    def test_mixture_log_density_parity(self):
        ...
        self.assertTrue(np.allclose(ll, ll_legacy, atol=1.0e-10),
                        'max err %g' % np.abs(ll - ll_legacy).max())
    ...
    def test_exponential_geometric_parity(self):
        comps = [CompositeDistribution((ExponentialDistribution(1.0 + k), GeometricDistribution(0.2 + 0.3 * k)))
                 for k in range(2)]
    ...
    def test_em_trajectory_matches_legacy(self):
        ...
        for _ in range(3):
            ...
            self.assertTrue(np.allclose(ll_k, ll_l, atol=1.0e-8),
                            'EM step diverged: max err %g' % np.abs(ll_k - ll_l).max())
            self.assertTrue(np.allclose(np.asarray(m_kernel.w, dtype=float),
                                        np.asarray(m_legacy.w, dtype=float), atol=1.0e-10))

    def test_fit_converges(self):
        est = make_estimator()
        model, ll = self.compiled.fit(self.enc, est, max_its=60, delta=1.0e-7,
...
class KernelsBenchmark(unittest.TestCase):
    ...
    def test_benchmark_vs_legacy(self):
        ...
        cm = CompiledMixture(model)
        t0 = time.time(); enc = cm.encode(data); t_enc_k = time.time() - t0
        t0 = time.time(); legacy_enc = model.dist_to_encoder().seq_encode(data); t_enc_l = time.time() - t0

        cm.seq_log_density(enc)  # warm-up compile
        t0 = time.time(); ll_k = cm.seq_log_density(enc); t_k = time.time() - t0
        t0 = time.time(); ll_l = model.seq_log_density(legacy_enc); t_l = time.time() - t0
        self.assertTrue(np.allclose(ll_k, ll_l, atol=1.0e-9))

        est = make_estimator()
        chunked = [(len(data), legacy_enc)]
        cm.em_step(enc, est)  # warm-up
        t0 = time.time(); cm.em_step(enc, est); t_em_k = time.time() - t0
        t0 = time.time(); seq_estimate(chunked, est, model); t_em_l = time.time() - t0
    ...
    def test_benchmark_sequence_heavy(self):
        ...
        comps = [SequenceDistribution(
            IntegerCategoricalDistribution(0, list(rng.dirichlet(np.ones(V) * 0.3))),
            len_dist=CategoricalDistribution({20: 0.3, 30: 0.4, 40: 0.3})) for _ in range(K)]
        model = MixtureDistribution(comps, [1.0 / K] * K)
        data = model.sampler(seed=1).sample(size=n)
        est = MixtureEstimator([SequenceEstimator(IntegerCategoricalEstimator(min_val=0, max_val=V - 1),
                                                  len_estimator=CategoricalEstimator())] * K)

        cm = CompiledMixture(model)
        enc = cm.encode(data)
        lenc = model.dist_to_encoder().seq_encode(data)
        cm.seq_log_density(enc); cm.em_step(enc, est)  # warm-up

        t0 = time.time(); ll_k = cm.seq_log_density(enc); t_k = time.time() - t0
        t0 = time.time(); ll_l = model.seq_log_density(lenc); t_l = time.time() - t0
        self.assertTrue(np.allclose(ll_k, ll_l, atol=1.0e-9))
        t0 = time.time(); cm.em_step(enc, est); t_em_k = time.time() - t0
        t0 = time.time(); seq_estimate([(n, lenc)], est, model); t_em_l = time.time() - t0
