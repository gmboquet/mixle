...
class LLDAEngineTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.est = LLDAEstimator([CategoricalEstimator(), CategoricalEstimator()], num_alphas=3,
                                 gamma_threshold=1.0e-10)
    ...
    def test_estep_parity(self):
        ...
        for name, engine in self.engines:
            with self.subTest(engine=name):
                ...
                self.assertTrue(np.allclose(np.asarray(hv[2]), np.asarray(value[2]), atol=1.0e-6),
                                '%s doc_counts differ' % name)
                self.assertTrue(np.allclose(np.asarray(hv[3]), np.asarray(value[3]), atol=1.0e-6),
                ...
                for ha, ea in zip(hv[4], value[4]):
                    ...
                    self.assertTrue(np.allclose(hvec, evec, atol=1.0e-6),
