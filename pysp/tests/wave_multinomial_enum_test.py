...
Covers MultinomialDistribution (pysp/stats/catmultinomial.py) and
IntegerMultinomialDistribution (pysp/stats/intmultinomial.py) against brute force on a
...
class MultinomialEnumeratorTestCase(unittest.TestCase):
    ...
    def test_complete_and_matches_brute_force(self):
        ...
        np.testing.assert_allclose([lp for _, lp in items], [lp for _, lp in self.brute], atol=TOL,
...
class IntegerMultinomialEnumeratorTestCase(unittest.TestCase):
    ...
    def test_top_k_matches_brute_force_top_k(self):
        ...
        self.assertGreater(cutoff, (self.BRUTE_MAX_N + 1) * np.log(0.5) + TOL,
                           'brute-force bound too small to certify the top-k')
        np.testing.assert_allclose([lp for _, lp in self.items],
                                   [lp for _, lp in self.brute[:self.TOP_K]], atol=TOL,
                                   err_msg='score sequence mismatch')
        full_tiers = tiers(self.brute)
