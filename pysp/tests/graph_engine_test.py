...
class GraphEngineParityTestCase(unittest.TestCase):
    ...
    def _dists(self):
        return [
            ...
            StochasticBlockGraphDistribution([[0.9, 0.1], [0.1, 0.6]], [0, 1, 0, 1, 1, 0],
                                             include_assignment_prior=True),
        ]
        ...
        for dist in self._dists():
            ...
            for name, engine in engines:
                with self.subTest(family=type(dist).__name__, engine=name):
                    ...
                    self.assertTrue(np.allclose(got, ref, atol=1.0e-7),
                                    '%s/%s backend score disagrees with seq_log_density'
