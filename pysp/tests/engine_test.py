...
from pysp.engines import NUMPY_ENGINE, NumpyEngine, SymbolicEngine, SymbolicExpression, TorchEngine, engine_of, \
    engine_with_precision, precision_name
...
class EngineTestCase(unittest.TestCase):
    ...
    def test_symbolic_engine_traces_array_expressions(self):
        ...
        np.testing.assert_allclose(
            np.asarray(engine.evaluate(col_sum, {'x': 1.0, 'y': 3.0}), dtype=float),
            np.asarray([4.0, 6.0]))
        np.testing.assert_allclose(
            np.asarray(engine.evaluate(product, {'x': 1.0, 'y': 3.0}), dtype=float),
            np.asarray([5.0, 11.0]))
        np.testing.assert_allclose(
