...
def _model():
    ...
    return IndPiHiddenMarkovModelDistribution(
        ...
        len_dist=CategoricalDistribution({3: 0.5, 4: 0.5}), use_numba=False)
...
class IndPiEngineEStepTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.est = IndPiHiddenMarkovEstimator([CategoricalEstimator(), CategoricalEstimator()],
    ...
    def test_engine_estep_parity(self):
        ...
        for name, engine in self.engines:
            with self.subTest(engine=name):
                ...
                v = acc.value()
                for k in (1, 2, 3):
                    self.assertTrue(np.allclose(np.asarray(hv[k]), np.asarray(v[k]), atol=1.0e-8),
