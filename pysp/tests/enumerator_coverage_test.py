...
def assert_matches_brute(test, dist, support, name):
    ...
    np.testing.assert_allclose(lps, [lp for _, lp in brute], atol=TOL,
                               err_msg='%s: score sequence mismatch' % name)
    ...
    for v, lp in items:
        ...
        test.assertAlmostEqual(lp, brute_by_key[freeze(v)], delta=TOL,
                               msg='%s: brute mismatch at %r' % (name, v))
