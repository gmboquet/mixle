...
import unittest
import numpy as np
from pysp.stats import JointMixtureDistribution, GaussianDistribution, GaussianEstimator
...
class JointMixtureEngineTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.est = JointMixtureEstimator([GaussianEstimator(), GaussianEstimator()],
    ...
    def test_engine_estep_parity(self):
        ...
        for name, engine in self.engines:
            with self.subTest(engine=name):
                ...
                for j in range(3):
                    self.assertTrue(np.allclose(np.asarray(hv[j]), np.asarray(value[j]), atol=1.0e-8),
