...
class KernelsExtBase(unittest.TestCase):
    ...
    def _assert_close(self, a, b, msg=''):
        if isinstance(a, list):
            ...
            np.testing.assert_allclose(np.asarray(a, dtype=float), np.asarray(b, dtype=float),
                                       rtol=self.RTOL, atol=self.ATOL, err_msg=msg)
    ...
    def check_seq_parity(self, model, data):
        ...
        self.assertTrue(np.allclose(ll_k, ll_l, rtol=1.0e-10, atol=1.0e-10),
        ...
        for k, comp in enumerate(model.components):
            ...
            self.assertTrue(np.allclose(ll_ck[:, k], cl, rtol=1.0e-10, atol=1.0e-10),
    ...
    def check_em_parity(self, model, est, data, steps=2):
        ...
        for it in range(steps):
            ...
            self._assert_close(dist_params(m_kernel), dist_params(m_legacy),
                               'em step %d ' % (it + 1))
...
class GammaKernelTest(KernelsExtBase):
    ...
    def test_in_composite(self):
        comps = [CompositeDistribution((GammaDistribution(2.0 + k, 1.0 + 0.5 * k), _cat(k)))
                 for k in range(2)]
...
class LogGaussianKernelTest(KernelsExtBase):
    ...
    def test_in_composite(self):
        comps = [CompositeDistribution((LogGaussianDistribution(0.5 * k, 0.4 + 0.2 * k), _cat(k)))
                 for k in range(2)]
...
class BinomialKernelTest(KernelsExtBase):
    ...
    def test_in_composite(self):
        comps = [CompositeDistribution((BinomialDistribution(0.25 + 0.4 * k, 15), _cat(k)))
                 for k in range(2)]
        model = MixtureDistribution(comps, [0.5, 0.5])
...
class DiagonalGaussianKernelTest(KernelsExtBase):
    ...
    def test_in_composite(self):
        comps = [CompositeDistribution((
            DiagonalGaussianDistribution([2.0 * k, -2.0 * k], [1.0, 0.5 + k]), _cat(k)))
            for k in range(2)]
        model = MixtureDistribution(comps, [0.5, 0.5])
        data = model.sampler(seed=42).sample(size=300)
        est = MixtureEstimator([CompositeEstimator((DiagonalGaussianEstimator(dim=2),
                                                    CategoricalEstimator()))] * 2)
        self.check_seq_parity(model, data)
        self.check_em_parity(model, est, data)

    def test_suff_stat_roundtrip(self):
        d = DiagonalGaussianDistribution([1.0, -1.0, 0.5], [0.8, 1.2, 0.5])
        self.check_suff_stat_roundtrip(d, DiagonalGaussianEstimator(dim=3),
                                       d.sampler(seed=43).sample(size=250))
...
class OptionalKernelTest(KernelsExtBase):
    ...
    def setUp(self):
        ...
        comps = [OptionalDistribution(GaussianDistribution(-3.0 + 3.0 * k, 1.0 + 0.5 * k), p=0.2 + 0.1 * k)
                 for k in range(3)]
    ...
    def test_in_composite(self):
        comps = [CompositeDistribution((
            OptionalDistribution(GaussianDistribution(4.0 * k, 1.0), p=0.3), _cat(k)))
            for k in range(2)]
        model = MixtureDistribution(comps, [0.5, 0.5])
        data = model.sampler(seed=52).sample(size=300)
        est = MixtureEstimator([CompositeEstimator((
            OptionalEstimator(GaussianEstimator(), est_prob=True), CategoricalEstimator()))] * 2)
        self.check_seq_parity(model, data)
        self.check_em_parity(model, est, data)
