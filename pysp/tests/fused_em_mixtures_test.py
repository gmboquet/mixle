...
from pysp.stats import (GaussianDistribution, GaussianEstimator, CategoricalDistribution,
                        HierarchicalMixtureDistribution, JointMixtureDistribution,
                        SemiSupervisedMixtureDistribution, DiracLengthMixtureDistribution,
                        HeterogeneousMixtureDistribution, GammaDistribution, GammaEstimator,
                        LogGaussianDistribution, LogGaussianEstimator,
                        MultivariateGaussianEstimator, seq_encode, seq_log_density_sum)
...
from pysp.stats.heterogenous_mixture import HeterogeneousMixtureEstimator
...
class FusedEMMixturesTestCase(unittest.TestCase):
    ...
    def _fused(self, est, data):
        std = optimize(data, est, max_its=12, delta=None, rng=np.random.RandomState(1),
                       out=io.StringIO())
        fused = optimize(data, est, max_its=12, delta=None, rng=np.random.RandomState(1),
    ...
    def _hmixture(self):
        dist = HierarchicalMixtureDistribution(
            [GaussianDistribution(-2.0, 1.0), GaussianDistribution(0.0, 1.0),
             GaussianDistribution(3.0, 1.0)],
            [0.5, 0.5], [[0.7, 0.2, 0.1], [0.1, 0.3, 0.6]],
            len_dist=CategoricalDistribution({2: 0.5, 3: 0.5}))
        est = HierarchicalMixtureEstimator(
            [GaussianEstimator(), GaussianEstimator(), GaussianEstimator()], num_mixtures=2,
    ...
    def _jmixture(self):
        ...
        dist = JointMixtureDistribution(
            ...
            [0.5, 0.5], [0.4, 0.3, 0.3], taus12, taus21)
        est = JointMixtureEstimator([GaussianEstimator(), GaussianEstimator()],
    ...
    def _dirac_length(self):
        dist = DiracLengthMixtureDistribution(
            CategoricalDistribution({2: 0.3, 3: 0.3, 4: 0.4}), p=0.6, v=0)
    ...
    def test_heterogenous_mixture(self):
        dist, est, data = self._heterogenous()
        self._parity(dist, est, data)
        self._fused(est, data)
        self._default_off(est)
    ...
    def _mvnmixture(self):
        dist = GaussianMixtureDistribution(mu=[[-2.0, -2.0], [2.0, 2.0]],
                                           sig2=[[1.0, 1.0], [1.0, 1.0]], w=[0.5, 0.5])
        est = GaussianMixtureEstimator([MultivariateGaussianEstimator(dim=2),
