...
from pysp.stats import seq_log_density_sum, seq_encode
...
class FusedEmAssociationTestCase(unittest.TestCase):
    ...
    def _check_fused(self, est, data, max_its=6):
        m0 = optimize(data, est, max_its=max_its, delta=None, rng=np.random.RandomState(3),
                      reuse_estep_ll=False, out=_devnull())
        m1 = optimize(data, est, max_its=max_its, delta=None, rng=np.random.RandomState(3),
                      reuse_estep_ll=True, out=_devnull())
    ...
    def test_int_hidden_association_numpy_parity(self):
        ...
        data = [([(0, 2.0), (1, 1.0)], [(0, 3.0), (1, 2.0)]),
                ([(1, 1.0), (2, 2.0)], [(2, 1.0)]),
                ([(0, 1.0)], [(0, 1.0), (1, 1.0), (2, 2.0)]),
                ([(2, 3.0), (0, 1.0)], [(1, 2.0), (2, 1.0)])]
        dist = IntegerHiddenAssociationDistribution(state_prob_mat=state, cond_weights=cond,
                                                    alpha=0.30, use_numba=False)
        est = IntegerHiddenAssociationEstimator(num_vals=[3, 3], num_states=2, alpha=0.30,
                                                use_numba=False)
        self._check_parity(dist, est, data)
    ...
    def _int_hidden_fused_data(self):
        ...
        gdist = IntegerHiddenAssociationDistribution(state_prob, cond_w, prev_dist=init_dist,
                                                     len_dist=len_dist)
    ...
    def test_int_hidden_association_fused_numpy(self):
        ...
        est = IntegerHiddenAssociationEstimator(4, 2, prev_estimator=prev_est, len_estimator=len_est,
                                                use_numba=False)
        self._check_fused(est, data)
    ...
    def _sparse_setup(self, low_memory):
        ...
        len_dist = CompositeDistribution((CategoricalDistribution({2: 0.5, 3: 0.5}),
                                          CategoricalDistribution({3: 0.6, 4: 0.4})))
        dist = SparseMarkovAssociationDistribution(init_prob, cond_prob, alpha=0.1, len_dist=len_dist,
                                                   low_memory=low_memory)
        data = dist.sampler(seed=5).sample(size=30)
        len_est = CompositeEstimator((CategoricalEstimator(), CategoricalEstimator()))
        est = SparseMarkovAssociationEstimator(num_vals=nw, alpha=0.1, len_estimator=len_est,
                                               low_memory=low_memory)
