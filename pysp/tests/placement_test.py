...
from pysp.parallel import CalibrationCatalog, DeviceSpec, EncodedDataHandle, LocalEncodedData, Resources, \
    calibrate_resources, encoded_data, estimate_model_nbytes, is_encoded_data_handle, model_sharding_plan, plan
from pysp.stats import GaussianDistribution, GaussianEstimator, MixtureDistribution, MixtureEstimator, seq_encode, \
    seq_estimate, seq_initialize, seq_log_density_sum
...
class PlacementPlanningTestCase(unittest.TestCase):
    ...
    def test_plan_estimates_rows_and_bytes_from_model_encoder(self):
        ...
        placement = plan(data=data, model=model, estimator=model.estimator(),
                         resources=Resources.single_cpu(memory_bytes=10_000_000))
    ...
    def test_weighted_resources_get_proportional_chunks(self):
        ...
        placement = plan(data=data, model=GaussianDistribution(0.0, 1.0),
                         estimator=GaussianEstimator(), resources=resources)
    ...
    def test_manual_num_chunks_round_robins_over_weighted_devices(self):
        ...
        placement = plan(data=list(range(40)), model=GaussianDistribution(0.0, 1.0),
                         estimator=GaussianEstimator(), resources=resources, num_chunks=4)
    ...
    def test_memory_cap_splits_large_shard(self):
        ...
        placement = plan(data=data, model=model, estimator=model.estimator(),
                         resources=resources, safety_factor=1.0)
    ...
    def test_calibrate_resources_can_time_estep_and_em_workloads(self):
        ...
        estep = calibrate_resources(data, model, resources=resources, sample_size=10,
                                    repeats=1, workload='estep')
        em = calibrate_resources(data, model, resources=resources, estimator=GaussianEstimator(),
                                 sample_size=10, repeats=1, workload='em')
    ...
    def test_resources_json_round_trip_preserves_calibration(self):
        resources = Resources.from_specs((
            ...
            DeviceSpec('cuda:0', kind='cuda', memory_bytes=2_000_000, engine='torch',
                       throughput=40.0, precision='float32'),
        ))
    ...
    def test_calibration_catalog_records_model_workload_resources(self):
        ...
        calibrated = calibrate_resources(data, model, resources=resources,
                                         sample_size=10, repeats=1, workload='score',
                                         precision='float64', catalog=catalog)
        ...
        self.assertEqual(catalog.resources_for('GaussianDistribution', 'score', 'float64').devices,
                         calibrated.devices)
    ...
    def test_calibration_catalog_path_round_trip(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            calibrated = calibrate_resources(data, model, resources=resources,
                                             ...
                                             sample_size=10, repeats=1,
                                             workload='em', catalog_path=path)
...
class LocalEncodedDataTestCase(unittest.TestCase):
    ...
    def test_local_handle_dispatch_matches_serial_log_density_and_estimate(self):
        ...
        with LocalEncodedData(data, model=model, estimator=estimator,
    ...
    def test_local_handle_keeps_engine_resident_payloads_for_scoring(self):
        ...
        with LocalEncodedData(data, model=model, estimator=estimator,
                              engine=engine, sub_chunks=2) as enc:
    ...
    def test_local_handle_initialize_and_optimize_use_unified_protocol(self):
        ...
        with LocalEncodedData(data, model=start, estimator=estimator,
                              resources=Resources.local(num_cpus=2),
                              sub_chunks=2) as enc:
            initialized = seq_initialize(enc, estimator, np.random.RandomState(2), p=0.5)
            fitted = optimize(None, estimator, enc_data=enc, prev_estimate=start,
                              max_its=8, delta=None)
