...
class SamplerAccuracyTestCase(unittest.TestCase):
    ...
    def test_core_scalar_sampler_moments_match_parameterization(self):
        ...
        scalar_cases = [
            ...
            ("inverse_gaussian", stats.InverseGaussianDistribution(2.0, 3.0), 2.0, 2.0**3 / 3.0),
            (
                "gumbel",
                stats.GumbelDistribution(2.0, 1.5),
                2.0 + 1.5 * 0.5772156649015328606,
                (math.pi**2 / 6.0) * 1.5**2,
            ),
            (
                "half_normal",
                stats.HalfNormalDistribution(1.5),
                1.5 * math.sqrt(2.0 / math.pi),
                1.5**2 * (1.0 - 2.0 / math.pi),
            ),
