...
from pysp.stats import GaussianDistribution, GaussianEstimator, MixtureDistribution, MixtureEstimator, seq_encode, \
    seq_estimate
from pysp.utils.em import AcceleratedEM, AnnealedEM, ConditionalMaximizationEM, GeneralizedEM, HardEM, \
    IncrementalEM, MonteCarloEM, OnlineEM, PosteriorTransformEM, RestartEM, StandardEM, VariationalEM, \
    observed_log_likelihood, run_em
...
def _assert_mixture_close(test_case, actual, expected):
    ...
    for got, exp in zip(actual.components, expected.components):
        np.testing.assert_allclose([got.mu, got.sigma2], [exp.mu, exp.sigma2],
...
class EMStrategiesTestCase(unittest.TestCase):
    ...
    def test_accelerated_em_accepts_objective_improving_proposal(self):
        ...
        result = AcceleratedEM(proposal, step_factors=(1.0,)).step(
            enc, estimator, model, objective=objective)
