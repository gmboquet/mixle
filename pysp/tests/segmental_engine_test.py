...
from pysp.stats import (SegmentalHiddenMarkovModelDistribution, GaussianDistribution,
    ...
    def test_engine_estep_parity(self):
        ...
        for name, engine in self.engines:
            with self.subTest(engine=name):
                ...
                for k in (1, 2, 3):
                    self.assertTrue(np.allclose(np.asarray(hv[k]), np.asarray(value[k]), atol=1.0e-8),
