...
class TreeHmmEngineTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dist = TreeHiddenMarkovModelDistribution(topics=topics, w=w, transitions=trans,
                                                      terminal_level=6, use_numba=False)
        self.trees = [
    ...
    def test_engine_estep_parity(self):
        ...
        for name, engine in self.engines:
            with self.subTest(engine=name):
                ...
                self.assertTrue(np.allclose(np.asarray(hv[1]), np.asarray(value[1]), atol=1.0e-8),
                                '%s init_counts differ' % name)
                self.assertTrue(np.allclose(np.asarray(hv[2]), np.asarray(value[2]), atol=1.0e-8),
                                '%s state_counts differ' % name)
                self.assertTrue(np.allclose(np.asarray(hv[3]), np.asarray(value[3]), atol=1.0e-8),
                                '%s trans_counts differ' % name)
                for ha, ea in zip(hv[4], value[4]):
                    self.assertTrue(np.allclose(np.asarray(ha[1]), np.asarray(ea[1]), atol=1.0e-7),
