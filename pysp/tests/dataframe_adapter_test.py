...
from pysp.stats import CategoricalDistribution, CompositeDistribution, GaussianDistribution, RecordDistribution, \
    GaussianEstimator, RecordEstimator, field, dataframe_records, seq_encode, seq_encode_dataframe, seq_log_density_sum
...
class DataFrameAdapterTestCase(unittest.TestCase):
    ...
    def test_seq_encode_dataframe_matches_list_encoding_for_scalar_model(self):
        ...
        np.testing.assert_allclose(seq_log_density_sum(enc_df, model),
                                   seq_log_density_sum(enc_list, model),
                                   rtol=0, atol=0)
    ...
    def test_dataframe_records_can_return_dict_rows(self):
        ...
        self.assertEqual(dataframe_records(df, fields=['x', 'label'], as_dict=True),
    ...
    def test_optimize_accepts_dataframe_fields(self):
        ...
        from_df = optimize(df, est, fields='x', prev_estimate=start, max_its=1, delta=None,
                           out=io.StringIO(), print_iter=100)
        from_list = optimize(data, est, prev_estimate=start, max_its=1, delta=None,
                             out=io.StringIO(), print_iter=100)
    ...
    def test_optimize_accepts_dataframe_fields_with_resources(self):
        ...
        placed = optimize(df, est, fields='x', prev_estimate=start, max_its=1, delta=None,
                          resources=Resources.local(num_cpus=2), sub_chunks=2,
                          out=io.StringIO(), print_iter=100)
        serial = optimize(data, est, prev_estimate=start, max_its=1, delta=None,
                          out=io.StringIO(), print_iter=100)
