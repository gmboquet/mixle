...
class HeterogeneousPCFGTestCase(unittest.TestCase):
    ...
    def test_sequence_valued_terminal_emissions(self):
        model = HeterogeneousPCFGDistribution(
            ...
            terminal_rules={
                'A': [(SequenceDistribution(GaussianDistribution(-2.0, 0.5),
                                            len_dist=CategoricalDistribution({1: 1.0})), 1.0)],
                'B': [(SequenceDistribution(GaussianDistribution(2.0, 0.5),
                                            len_dist=CategoricalDistribution({2: 1.0})), 1.0)],
            },
            start='S',
        )
        ...
        est = HeterogeneousPCFGEstimator(
            binary_rules={'S': [('A', 'B', 1.0)]},
            terminal_rules={
                'A': [(SequenceEstimator(GaussianDistribution(-1.0, 2.0).estimator(),
                                         len_estimator=CategoricalDistribution({1: 1.0}).estimator()), 1.0)],
                'B': [(SequenceEstimator(GaussianDistribution(1.0, 2.0).estimator(),
                                         len_estimator=CategoricalDistribution({2: 1.0}).estimator()), 1.0)],
            },
            start='S',
            pseudo_count=1.0,
        )
    ...
    def test_discrete_enumerator_and_quantized_index(self):
        ...
        self.assertEqual([u[0] for u in top], [
            ['a', 'x'],
            ['a', 'y'],
            ['b', 'x'],
            ['b', 'y'],
    ...
    def test_recursive_pcfg_quantized_index_uses_bounded_dp(self):
        ...
        self.assertEqual([u[0] for u in index.iter_from()], [
            ['z'],
            ['z', 'a'],
            ['z', 'a', 'a'],
            ['z', 'a', 'a', 'a'],
    ...
    def test_induced_estimator_learns_overcomplete_sparse_grammar(self):
        ...
        est = InducedHeterogeneousPCFGEstimator(
            max_nonterminals=3,
            terminal_estimators=[a_dist.estimator(pseudo_count=1.0),
                                 b_dist.estimator(pseudo_count=1.0)],
