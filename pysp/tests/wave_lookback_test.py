...
def make_dist(mod):
    ...
    return mod.LookbackHiddenMarkovDistribution(
        [t1, t2], w=[0.6, 0.4], transitions=[[0.8, 0.2], [0.3, 0.7]], lag=1,
...
def make_estimator(mod, with_len=True):
    ...
    """
    init_est = SequenceEstimator(IntegerCategoricalEstimator(min_val=0, max_val=2, pseudo_count=0.1),
    ...
    return mod.LookbackHiddenMarkovEstimator(
        [topic_est] * 2, lag=1, init_estimators=[init_est] * 2,
    ...
    def test_both_modules_define_protocol_classes(self):
        for mod in MODULES:
            with self.subTest(module=mod.__name__):
                for cls_name in ('LookbackHiddenMarkovDistribution', 'LookbackHiddenMarkovSampler',
                                 'LookbackHiddenMarkovEstimatorAccumulator',
                                 'LookbackHiddenMarkovEstimatorAccumulatorFactory',
                                 'LookbackHiddenMarkovEstimator', 'LookbackHiddenMarkovDataEncoder'):
    ...
    def test_protocol_methods_present(self):
        for mod in MODULES:
            with self.subTest(module=mod.__name__):
                ...
                for name in ('log_density', 'seq_log_density', 'seq_encode', 'dist_to_encoder',
                             'sampler', 'estimator'):
                    self.assertTrue(callable(getattr(dist, name)))
                for name in ('update', 'initialize', 'seq_update', 'seq_initialize', 'combine',
                             'value', 'from_value', 'acc_to_encoder'):
