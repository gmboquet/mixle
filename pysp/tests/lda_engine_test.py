...
from pysp.stats import (LDADistribution, CategoricalDistribution, CategoricalEstimator)
...
class LDAEngineTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dist = LDADistribution(topics, alpha=[1.0, 1.0, 1.0],
    ...
    def test_engine_estep_parity(self):
        ...
        for name, engine in self.engines:
            with self.subTest(engine=name):
                ...
                self.assertTrue(np.allclose(np.asarray(hv[1]), np.asarray(value[1]), atol=1.0e-7),
                                '%s sum_of_logs differ' % name)
                self.assertTrue(np.allclose(np.asarray(hv[2]), np.asarray(value[2]), atol=1.0e-8),
                                '%s doc_counts differ' % name)
                self.assertTrue(np.allclose(np.asarray(hv[3]), np.asarray(value[3]), atol=1.0e-7),
                                '%s topic_counts differ' % name)
                for ha, ea in zip(hv[4], value[4]):
                    self.assertTrue(np.allclose(np.asarray(ha[0]), np.asarray(ea[0]), atol=1.0e-7),
