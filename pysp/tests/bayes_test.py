"""Tests for the Bayesian (conjugate-prior / variational) machinery in pysp.stats.
...
class PriorDensityTestCase(unittest.TestCase):
    ...
    # Beta/Dirichlet priors are not part of pysp.stats -- only the conjugate
    ...
    # helpers) have no pysp.stats behavior to assert, so they are not included.
    ...
    def test_support_boundaries_match_scalar_and_raise_on_seq_encode(self):
        ...
        # ...but pysp.stats validates the support at seq_encode time.
        for dist, data in [
...
class MarkovChainTestCase(unittest.TestCase):
    ...
    # pysp.stats Dirichlet import, and the closed-form posterior counts are
    # already covered by
    # pysp/tests/stats_bayes_markov_test.py::test_markov_chain_posterior_closed_form.
...
class UtilityTestCase(unittest.TestCase):
    ...
    def test_stirling2(self):
        # The Poisson.moment() helper is not part of pysp.stats; the underlying
...
class IntegerCategoricalConventionTestCase(unittest.TestCase):
    """pysp.stats IntegerCategorical uses the (min_val, p_vec) order and the
