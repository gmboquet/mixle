...
dependency, so it lives in pysp proper: the edge-preserving field priors (``TotalVariation`` / ``Potts``
in :mod:`pysp.ppl.priors`) use it, and so does the PDE divergence-form assembly (now in the
``pysparkplug-pde`` package, which imports it from here). Kept here so neither side depends on the other.
"""
