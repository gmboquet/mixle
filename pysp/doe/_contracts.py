...
:func:`pysp.doe.bayesopt.register_acquisition`, optimality criteria through
:func:`pysp.doe.optimal.register_criterion`, and the GP surrogate is passed in as a duck-typed
...
class Acquisition(Protocol):
    ...
    *merit* array that the proposal loop maximizes over the candidate set. It is called as
    ...
    EI/PI, ``kappa`` for UCB). Built-ins :func:`pysp.doe.bayesopt.expected_improvement`,
...
class Surrogate(Protocol):
    ...
    :class:`pysp.models.gaussian_process.GaussianProcessRegressor`: ``fit`` trains in place (it may
...
class Criterion(Protocol):
    ...
    :func:`pysp.doe.optimal.optimal_design` maximizes over candidate designs. It is called as
    ``fn(info, *, ref)`` where ``ref`` is an optional reference model matrix (used by I-optimality).
    Built-ins :func:`pysp.doe.optimal.d_criterion`, :func:`a_criterion`, and :func:`i_criterion`
