...
from pysp.bstats.pdist import ProbabilityDistribution, SequenceEncodableAccumulator, \
	ParameterEstimator, DataFrameEncodableDistribution, DataFrameEncodableAccumulator
...
class CompositeDistribution(ProbabilityDistribution):
	...
	def set_prior(self, prior):
		...
		if len(prior.dists) != self.count:
			raise ValueError('CompositeDistribution.set_prior expected %d priors but got %d.' % (self.count, len(prior.dists)))
...
class CompositeEstimator(ParameterEstimator):
	...
	def set_prior(self, params):
		...
		if len(params.dists) != self.count:
			raise ValueError('CompositeEstimator.set_prior expected %d priors but got %d.' % (self.count, len(params.dists)))
