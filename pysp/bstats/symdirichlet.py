...
class SymmetricDirichletDistribution(SequenceEncodableDistribution):
	...
	def __init__(self, alpha: float, dim: Optional[int] = None):
...
class SymmetricDirichletSampler(object):
	...
	def sample(self, size: Optional[int] = None) -> Union[float, np.ndarray]:
		...
		if n is None:
			raise ValueError('SymmetricDirichletSampler requires SymmetricDirichletDistribution(alpha, dim=...) with a specified dimension.')
