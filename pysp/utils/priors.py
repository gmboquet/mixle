...
class MarkovChainPrior:
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            'transitions': {} if self.transitions is None else {
                key: as_prior_dict(value) for key, value in self.transitions.items()
            },
...
def normal_gamma(mu0: float = 0.0, kappa: float = 0.0,
                 alpha: float = 1.0, beta: float = 0.0) -> NormalGammaPrior:
...
def beta(alpha: float, beta: float, parameter: Optional[str] = None) -> BetaPrior:
    """Create a Beta prior for a unit-interval parameter."""
    return BetaPrior(alpha=alpha, beta=beta, parameter=parameter)


def gamma(shape: float, rate: float, parameter: Optional[str] = None) -> GammaPrior:
...
def conditional(conditions: Mapping[Any, Any],
                default: Optional[Any] = None,
                given: Optional[Any] = None) -> ConditionalPrior:
...
def mixture(components: Sequence[Any] = (), weights: Optional[Any] = None) -> MixturePrior:
    """Create a Mixture prior over component and weight priors."""
    return MixturePrior(components=components, weights=weights)


def markov_chain(initial: Optional[Any] = None,
                 transitions: Optional[Mapping[Any, Any]] = None,
                 length: Optional[Any] = None) -> MarkovChainPrior:
...
def optional(observed: Optional[Any] = None, missing: Optional[Any] = None) -> OptionalPrior:
