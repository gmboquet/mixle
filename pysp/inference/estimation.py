...
def _em_step_fn(engine: Any | None, strategy: Any | None = None):
    ...
    (``pysp.inference.em``) or any callable, which is how alternative E-steps
...
def optimize(
    ...
    Args:
        ...
        resources (Optional[Any]): Optional planner resources. When supplied with raw data, optimize encodes through
            ...
            contract.
        placement (Optional[Any]): Optional explicit placement produced by ``pysp.utils.parallel.planner.plan``.
        ...
        strategy (Optional[Any]): Optional EM strategy from ``pysp.inference.em`` (e.g. ``AnnealedEM``,
    ...
    if enc_data is None:
        ...
        if resources is not None or placement is not None or backend_name != "local":
            from pysp.utils.parallel.planner import encoded_data, is_encoded_data_handle
