...
* :mod:`pysp.inference.diagnostics` (re-exported as :func:`rhat` / :func:`ess`) — convergence
  diagnostics over plain ``(n_chains, n_draws, d)`` arrays.

The backend registry lives in :mod:`pysp.inference.backends` (`register, don't branch`); each backend
self-registers at import, behind a try/except so a missing optional engine never breaks
``import pysp.inference``.
"""
...
def _nuts_numpy(
    ...
    *,
    ...
    from pysp.inference.mcmc import nuts as _nuts
...
def _nuts_numba(
    ...
    *,
    ...
    from pysp.inference.mcmc.nuts_numba import nuts_numba as _nuts_numba_core
...
def nuts_torch(
    ...
    **Performance note (be deliberate about when to use this):** on CPU this is typically *slower*
    ...
    """
    from pysp.inference.mcmc.nuts_torch import nuts_torch as _nuts_torch
...
# never breaks ``import pysp.inference``.
# --------------------------------------------------------------------------------------------
