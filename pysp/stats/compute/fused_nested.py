...
It reuses the :class:`~pysp.stats.compute.fused_codegen.LeafTemplate` machinery by giving every leaf node a
...
:func:`~pysp.stats.compute.fused_codegen.analyze` declines, so the flat fast path is never perturbed.
"""
...
from pysp.stats.compute.fused_codegen import LeafTemplate, _njit, _template_for
...
def analyze_nested(model: Any) -> tuple[Any, _Ctx] | None:
    ...
    flat :func:`~pysp.stats.compute.fused_codegen.analyze`, so this only fires when that one declines.
    """
