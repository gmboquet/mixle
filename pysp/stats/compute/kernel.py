...
class GeneratedNumbaKernel(Kernel):
    ...
    def score(self, enc: Any) -> np.ndarray:
        ...
        from pysp.stats.compute.declarations import generated_numba_log_density
...
def _stacked_kernel_after_numba_decline(
    ...
    if components is None or log_w is None:
        return None
    try:
        from pysp.stats.compute.stacked import StackedMixtureKernel
...
def _generated_numba_kernel_available(dist: SequenceEncodableProbabilityDistribution) -> bool:
    from pysp.stats.compute.declarations import generated_numba_log_density_available
...
def _generated_numba_components_available(components: tuple) -> bool:
    ...
    if not all(type(component) is component_type for component in components):
        return False
    from pysp.stats.compute.declarations import generated_numba_stacked_available, generated_stacked_params
...
def kernel_for(
    ...
    if getattr(engine, "prefer_fused", False):
        from pysp.stats.compute.fused_codegen import FusedKernel, analyze, fusible_estep

        plan = analyze(dist)
        if plan is not None and (plan.num_components > 1 or len(plan.leaf_templates) > 1):
