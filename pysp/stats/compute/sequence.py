...
:mod:`pysp.stats.compute.pdist`. These were defined inline in ``pysp.stats.__init__``; they live here
so the fitting machinery (``pysp.inference``) can import them WITHOUT importing the whole
``pysp.stats`` package — which previously forced ``pysp.inference`` to resolve lazily to dodge a
half-initialized ``pysp.stats``. ``pysp.stats`` re-exports them, so the public
``pysp.stats.seq_estimate`` / ``seq_log_density_sum`` / … API is unchanged.
"""
