...
from pysp.stats.pdist import DataSequenceEncoder, DistributionSampler, ParameterEstimator, \
    SequenceEncodableProbabilityDistribution, SequenceEncodableStatisticAccumulator, StatisticAccumulatorFactory, \
...
TerminalRuleInput = Union[Dict[Any, Sequence[Tuple[Any, float]]],
                          Sequence[Tuple[Any, Any, float]]]
EncodedPCFGData = Tuple[np.ndarray, Tuple[Any, ...]]


def _iter_binary_rules(binary_rules: Optional[BinaryRuleInput]) -> Iterable[Tuple[Any, Any, Any, float]]:
...
class HeterogeneousPCFGDistribution(SequenceEncodableProbabilityDistribution):
    ...
    def __init__(self,
                 binary_rules: Optional[BinaryRuleInput],
                 terminal_rules: TerminalRuleInput,
                 start: Optional[Any] = None,
                 nonterminals: Optional[Sequence[Any]] = None,
    ...
    def __str__(self) -> str:
        ...
        terminal = repr([
            (self.nonterminals[p], str(d), float(q))
            for p, d, q in self.terminal_rules
        ])
        return 'HeterogeneousPCFGDistribution(binary_rules=%s, terminal_rules=%s, start=%s, ' \
    ...
    def _inside_outside(self, terminal_log_density: np.ndarray) \
        ...
        for i in range(n):
            for rule_idx in range(self.num_terminal_rules):
                ...
                v = outside[i, i + 1, parent] + self.log_terminal_probs[rule_idx] \
                    + terminal_log_density[i, rule_idx] - ll
                if np.isfinite(v):
    ...
    def estimator(self, pseudo_count: Optional[float] = None) -> 'HeterogeneousPCFGEstimator':
...
class HeterogeneousPCFGEnumerator(DistributionEnumerator):
    ...
    def _terminal_stream(self, rule_idx: int, rule_lp: float):
        ...
        enum = child_enumerator(
            emission, 'HeterogeneousPCFGDistribution.terminal_rules[%d]' % rule_idx)
    ...
    def _stream_for_nonterminal(self, nt: int) -> BufferedStream:
        ...
        try:
            ...
            for rule_idx in self.dist.binary_by_parent[nt]:
                ...
                product = ProductEnumerator(
                    [left, right], combine=self._concat_children, offset=rule_lp)
...
class _HeterogeneousPCFGQuantizedIndexBuilder(object):
    """Build and lazily unrank a quantized derivation-count index."""

    def __init__(self, dist: HeterogeneousPCFGDistribution,
                 max_bits: float, bin_width_bits: float) -> None:
        if max_bits < 0:
        ...
        self.terminal_counts: List[List[int]] = [
            [0] * (self.max_bin + 1) for _ in range(dist.num_nonterminals)]
        self.counts: List[List[int]] = []

    def _qcost(self, log_prob: float) -> Optional[int]:
        if log_prob == -np.inf:
            return None
    ...
    def _prepare_terminal_options(self) -> None:
        for rule_idx, (parent, emission, _) in enumerate(self.dist.terminal_rules):
            ...
            if rule_cost > self.max_bin:
                ...
                continue
            try:
                index = emission.quantized_index(
                    max_bits=self.max_bits, bin_width_bits=self.bin_width_bits)
    ...
    def _compute_counts(self) -> None:
            ...
            for rule_idx in range(self.dist.num_binary_rules):
                ...
                for left_bin, left_count in enumerate(self.counts[left]):
                    ...
                    for right_bin, right_count in enumerate(self.counts[right][:remaining + 1]):
                        if right_count:
                            next_counts[parent][rule_cost + left_bin + right_bin] += \
        ...
        raise EnumerationError(
            self.dist,
            reason='quantized PCFG counting did not converge within the bit bound')
    ...
    def _unrank_nonterminal(self, nt: int, bin_id: int, offset: int) -> List[Any]:
        ...
        for rule_idx in self.dist.binary_by_parent[nt]:
            ...
            for left_bin in range(rem + 1):
                ...
                return self._unrank_nonterminal(left, left_bin, left_offset) + \
                    self._unrank_nonterminal(right, right_bin, right_offset)
    ...
    def build(self) -> LazyQuantizedEnumerationIndex:
        ...
        counts = {b: self.counts[self.dist.start_idx][b]
        ...
        return LazyQuantizedEnumerationIndex(
            counts, bin_width_bits=self.bin_width_bits, max_bits=self.max_bits,
            truncated=self.truncated, getter=getter)


class HeterogeneousPCFGSampler(DistributionSampler):
    """Sampler for HeterogeneousPCFGDistribution."""

    def __init__(self, dist: HeterogeneousPCFGDistribution, seed: Optional[int] = None,
                 max_depth: int = 100, max_steps: int = 10000) -> None:
        self.dist = dist
...
class HeterogeneousPCFGAccumulator(SequenceEncodableStatisticAccumulator):
    ...
    def seq_update(self, x: EncodedPCFGData, weights: np.ndarray,
                   estimate: HeterogeneousPCFGDistribution) -> None:
        lengths, enc_by_rule = x
        total = int(lengths.sum())
    ...
    def combine(self, suff_stat: Tuple[np.ndarray, np.ndarray, Sequence[Any]]) \
...
class HeterogeneousPCFGEstimator(ParameterEstimator):
    ...
    def estimate(self, nobs: Optional[float],
                 suff_stat: Tuple[np.ndarray, np.ndarray, Sequence[Any]]) -> HeterogeneousPCFGDistribution:
        terminal_counts, binary_counts, emission_values = suff_stat
        emissions = [
    ...
    * every terminal rule ``A -> terminal_family_j``.
    ...
    def __init__(self,
        ...
        self.nonterminals = [start] + [
            '%s%d' % (nonterminal_prefix, i) for i in range(1, self.max_nonterminals)
        ]
        self.raw_binary = self._make_binary_rules()
        self.raw_terminal = self._make_terminal_rules(self.terminal_family_estimators)
        prior_terminal_rules = [(parent, est, prob) for parent, est, prob in self.raw_terminal]
        self._prior = HeterogeneousPCFGDistribution(
            self.raw_binary, prior_terminal_rules, start=self.start,
            nonterminals=self.nonterminals, name=self.name)
        self.terminal_estimators = [est for _, est, _ in self.raw_terminal]
    ...
    def initial_model(self,
                      ...
                      jitter: float = 0.0) -> HeterogeneousPCFGDistribution:
        ...
        if len(terminal_distributions) == self.num_terminal_families:
            emissions = [
                terminal_distributions[j]
                for _ in self.nonterminals
                for j in range(self.num_terminal_families)
            ]
    ...
    def _normalize_rule_probs(self, term_probs: np.ndarray, bin_probs: np.ndarray,
                              use_prior_on_empty: bool = True) -> None:
        for parent in range(self._prior.num_nonterminals):
            term_idx = self._prior.terminal_by_parent[parent]
            bin_idx = self._prior.binary_by_parent[parent]
            total = 0.0
