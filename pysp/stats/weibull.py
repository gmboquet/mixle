...
class WeibullDistribution(SequenceEncodableProbabilityDistribution):
    ...
    def __str__(self) -> str:
        return 'WeibullDistribution(%s, %s, name=%s, keys=%s)' % (
            repr(self.shape), repr(self.scale), repr(self.name), repr(self.keys))
    ...
    def seq_log_density(self, x: Tuple[np.ndarray, np.ndarray]) -> np.ndarray:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            rv = self.log_shape - self.log_scale + (self.shape - 1.0) * (lx - self.log_scale) \
    ...
    def estimator(self, pseudo_count: Optional[float] = None) -> 'WeibullEstimator':
        ...
        return WeibullEstimator(pseudo_count=pseudo_count, suff_stat=(mean, second),
...
class WeibullSampler(DistributionSampler):
    ...
    def __init__(self, dist: WeibullDistribution, seed: Optional[int] = None) -> None:
...
class WeibullAccumulator(SequenceEncodableStatisticAccumulator):
    ...
    def update(self, x: float, weight: float, estimate: Optional[WeibullDistribution]) -> None:
        if x < 0.0:
    ...
    def seq_update(self, x: Tuple[np.ndarray, np.ndarray], weights: np.ndarray,
                   estimate: Optional[WeibullDistribution]) -> None:
        xx, _ = x
...
class WeibullEstimator(ParameterEstimator):
    ...
    def __init__(self, pseudo_count: Optional[float] = None,
                 ...
                 min_shape: float = 1.0e-3, max_shape: float = 1.0e3,
                 min_scale: float = 1.0e-12, name: Optional[str] = None,
                 keys: Optional[str] = None) -> None:
    ...
    def estimate(self, nobs: Optional[float],
                 suff_stat: Tuple[float, float, float]) -> WeibullDistribution:
