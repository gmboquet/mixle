...
The library's :class:`~pysp.stats.HawkesProcessDistribution` uses an *exponential* triggering kernel,
...
class PowerLawHawkesDistribution(SequenceEncodableProbabilityDistribution):
    ...
    def __init__(self, mu, A, c, p, window, *, alpha=0.0, mark_dist=None, name=None, keys=None):
        ...
        self.mark_dist = mark_dist  # an optional pysp distribution the marks are drawn from (None => unmarked)
        self.name = name
        self.keys = keys
