...
def _networkx_like_to_adjacency(graph: Any) -> Tuple[np.ndarray, Optional[np.ndarray]]:
...
def _as_assignments(assignments: Optional[Any], n: int) -> Optional[np.ndarray]:
...
def _extract_observation(x: Any, directed: bool = False,
                         fallback_assignments: Optional[Any] = None) -> GraphObservation:
...
def _normalize_prior(block_prior: Optional[Any], num_blocks: int) -> np.ndarray:
    if num_blocks <= 0:
...
class GraphDataEncoder(DataSequenceEncoder):
    ...
    def __init__(self, directed: bool = False,
                 fallback_assignments: Optional[Any] = None) -> None:
        self.directed = bool(directed)
        self.fallback_assignments = None if fallback_assignments is None else tuple(
            int(u) for u in np.asarray(fallback_assignments, dtype=np.int64))

    def __str__(self) -> str:
        return 'GraphDataEncoder(directed=%s)' % repr(self.directed)

    def __eq__(self, other: object) -> bool:
        return isinstance(other, GraphDataEncoder) and \
            self.directed == other.directed and self.fallback_assignments == other.fallback_assignments
