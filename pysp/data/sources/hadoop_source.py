...
``pip install pysparkplug[hadoop]``. ``read_remote(path, format=...)`` opens the object store path and
    ...
    **storage_options: Any,
    ...
    def factory():
        with _fsspec.open(path, "rb" if fmt in {"parquet", "feather"} else "rt", **storage_options) as fh:
            if fmt == "parquet":
                from pysp.data.sources.arrow_source import _require_arrow, _table_records  # noqa: PLC0415
            ...
            if fmt in _TEXT:
                ...
                from pysp.data.sources import text_source
