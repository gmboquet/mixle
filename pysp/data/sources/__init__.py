...
Every connector returns a :class:`~pysp.data.core.DataSource` and is *lazy + optional*: nothing here is
...
def open(kind: str, *args: Any, **kwargs: Any) -> Any:
    ...
    driver raises a clear ``pip install pysparkplug[extra]`` message.
    """
    if kind not in _KINDS:
        raise ValueError("unknown source kind %r; known kinds: %s" % (kind, ", ".join(sorted(_KINDS))))
    module_name, reader_name = _KINDS[kind]
    module = importlib.import_module("pysp.data.sources." + module_name)
