...
if __name__ == "__main__":
    ...
    d = MultinomialDistribution(
        CategoricalDistribution({"a": 0.5, "b": 0.3, "c": 0.2}), len_dist=CategoricalDistribution({8: 1.0})
    )
    fit = estimate(
        d.sampler(seed=1).sample(3000),
    ...
    fit = estimate(
        d.sampler(seed=1).sample(3000), IntegerMultinomialEstimator(min_val=0, len_estimator=CategoricalEstimator())
    )
