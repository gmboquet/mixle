...
if __name__ == "__main__":
    ...
    d = CompositeDistribution(
        (GaussianDistribution(2.0, 1.0), CategoricalDistribution({"a": 0.7, "b": 0.3}), PoissonDistribution(5.0))
    )
    fit = estimate(
        d.sampler(seed=1).sample(4000),
    ...
    fit = estimate(
        d.sampler(seed=1).sample(3000), SequenceEstimator(CategoricalEstimator(), len_estimator=PoissonEstimator())
    )
    ...
    fit = estimate(d.sampler(seed=1).sample(4000), OptionalEstimator(GaussianEstimator(), est_prob=True))
    print("  fit: %s" % fit)
    ...
    data = list(GaussianDistribution(-5.0, 1.0).sampler(seed=1).sample(2000)) + list(
        GaussianDistribution(5.0, 1.0).sampler(seed=2).sample(2000)
    )
