...
named-field Record / DictRecord models. Self-contained random data only.
...
(latent alignments), ``latent_variable_models_example.py`` (LDA / IBP / PPCA admixtures) -- or their
own galleries (``gallery_graphs_example.py`` for the random-graph families). The grammar
(HeterogeneousPCFG) and sparse-association (SparseMarkovAssociation) models are exercised in
...
if __name__ == "__main__":
    ...
    d = MarkovChainDistribution(
        {"a": 0.5, "b": 0.5}, {"a": {"a": 0.9, "b": 0.1}, "b": {"a": 0.2, "b": 0.8}}, len_dist=PoissonDistribution(6.0)
    )
    m = optimize(
    ...
    m = optimize(d.sampler(1).sample(2000), IndianBuffetProcessEstimator(num_features=4), max_its=20, rng=RNG())
    ...
    rng = RNG()
    base = rng.randint(0, 2, size=2000)
    icl_data = [[int(base[i]), int(base[i] ^ (rng.rand() < 0.1)), int(rng.rand() < 0.5)] for i in range(2000)]
    m = optimize(icl_data, IntegerChowLiuTreeEstimator(num_features=3, num_states=2), max_its=1, rng=RNG())
    print(
        "  learned dependencies (feature, parent): %s"
